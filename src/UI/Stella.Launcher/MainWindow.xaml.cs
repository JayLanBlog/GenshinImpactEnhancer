...
public partial class MainWindow : Window
{
    ...
    private void InjectDll(string gameDir)
    {
        ...
        var dllPath = Path.Combine(exeDir, "Stella.Native.Injector.dll");
        if (!File.Exists(dllPath))
            dllPath = Path.GetFullPath(Path.Combine(exeDir, @"..\..\..\..\..\src\Native\Stella.Native.Injector\x64\Release\Stella.Native.Injector.dll"));
        if (!File.Exists(dllPath))
